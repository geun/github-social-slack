  ...
  switch (event.type) {
    case 'PullRequestEvent':
      if (event.payload.action == 'closed') {
        var text_fmt = 'Merged/Closed pull request ' + event.payload.pull_request.html_url;
      } else {
        var text_fmt = 'Opened pull request ' + event.payload.pull_request.html_url;
      }
      break;
    case 'CreateEvent':
    ...
    case 'WatchEvent':
      ...
      break;
    case 'DeleteEvent':
      var text_fmt = 'Deleted branch ' +  '`'+event.payload.ref+'`' + ' from ' + friendly(event.repo.url);
      break;
    case 'PushEvent':
      ...
      var text_fmt = 'Pushed to `' + branch + '` at ' + friendly(event.repo.url) + '\n\r' + commit_url;
      break;
    default:
